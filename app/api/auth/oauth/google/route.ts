...
import { exchangeGoogleCode, googleAuthUrl, upsertUserFromOAuth } from "@/lib/auth/oauth";
import { issueTokens, setAuthCookies } from "@/lib/auth/session";
import { authConfig } from "@/lib/auth/config";

export const runtime = "nodejs";

/** GET — start web OAuth flow (redirects to Google). */
...
/**
 * POST — mobile flow: client performs OAuth itself, sends authorization code
 * { code, redirectUri } => returns tokens + user.
 */
export async function POST(req: NextRequest) {
  const parsed = await parseBody(req, oauthCallbackSchema);
  if (!parsed.ok) return parsed.response;

  const redirectUri = parsed.data.redirectUri ?? `${authConfig.appUrl}/api/auth/oauth/google/callback`;
  try {
    const profile = await exchangeGoogleCode(parsed.data.code, redirectUri);
    ...
    return fail("Échec de la connexion Google", 400, "OAUTH_FAILED");
  }
}
